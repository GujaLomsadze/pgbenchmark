...
class ParallelBenchmark:
    ...
    def __init__(
        ...
        if not self._is_sqlalchemy_url and not self._is_psycopg2_params:
            raise TypeError(
                "db_connection_info must be a dict (for psycopg2 params) or a str (for SQLAlchemy URL)."
            )
        ...
        if self._is_sqlalchemy_url and (
            create_engine is None or sqlalchemy_text is None
        ):
            raise ImportError(
        ...
        self._is_running: bool = (
            False  # Flag to indicate if benchmark is currently running/iterable
        )
        ...
        self._total_run_duration: float = (
            0.0  # Duration measured from start to end of run() or iteration
        )
        ...
        try:
            ...
            if conn:
                try:
                    ...
                    print(
                        f"[Worker {worker_name}] Warning: Error closing psycopg2 connection: {type(e).__name__}: {e}",
                        file=sys.stderr,
                    )
                    sys.stderr.flush()
            if engine:
                try:
                    ...
                    print(
                        f"[Worker {worker_name}] Warning: Error disposing SQLAlchemy engine: {type(e).__name__}: {e}",
                        file=sys.stderr,
                    )
                    sys.stderr.flush()
    ...
    def __iter__(self):
        ...
        print(
            f"[MainProcess] Starting benchmark for iteration with {self.num_processes} processes, "
        ...
        if runs_for_each_process <= 0:
            print(
                "[MainProcess] Warning: number_of_runs_per_process is not positive, no work will be done."
            )
            sys.stdout.flush()
    ...
    def __next__(self):
        ...
        if not self._is_running:
            ...
            if self._completed_workers_count >= self.num_processes:
                ...
                raise RuntimeError(
                    "Benchmark iteration not started or is in an invalid state."
                )
            ...
            try:
                ...
                if item == WORKER_DONE_SENTINEL:
                    ...
                    if self._completed_workers_count == self.num_processes:
                        ...
                        self._total_run_duration = (
                            time.time() - self._benchmark_start_time
                        )  # Final duration
                        print(
                            "[MainProcess] All workers finished. Joining processes..."
                        )
                        sys.stdout.flush()
                        ...
                        for p in self._processes:
                            ...
                            if p.is_alive():
                                print(
                                    f"[MainProcess] Warning: Process {p.name} did not join cleanly, terminating.",
                                    file=sys.stderr,
                                )
                                sys.stderr.flush()
                    ...
                    if item.get(IS_RESULT):
                        ...
                        self._run_timestamps.append(
                            {SENT_AT: item[SENT_AT], DURATION: item[DURATION]}
                        )
                    else:  # Error item
                        self._errors.append(
                            item.get(ERROR, "Unknown error from worker")
                        )
        ...
        if self._completed_workers_count >= self.num_processes:
            ...
            raise RuntimeError(
                "Benchmark iterator finished unexpectedly before all workers signaled completion."
            )
    ...
    def iter_successful_results(self) -> Iterator[Dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"[MainProcess] Unexpected error during successful results iteration: {type(e).__name__}: {e}",
                file=sys.stderr,
            )
            sys.stderr.flush()
    ...
    def run(self, live_reporting: bool = False):
            ...
            for item in self:
                if isinstance(item, dict):
                    if item.get(IS_RESULT):
                        ...
                        if live_reporting:
                            print(
                                f"[{item[WORKER_NAME]}] Executed {item[SENT_AT]} UTC, took {item[DURATION]}s"
                            )
                            sys.stdout.flush()
                    else:  # Error item
                        # Errors are always reported live in blocking mode
                        print(
                            f"[MainProcess] Worker Error: {item.get(ERROR, 'Unknown error from worker')}"
                        )
                        sys.stdout.flush()
                else:
                    # Unexpected items from queue (should be handled in __next__ yielding an error dict)
                    print(
                        f"[MainProcess] Warning: Received unexpected item from queue during run(): {item}",
                        file=sys.stderr,
                    )
                    sys.stderr.flush()
        ...
        print(
            f"[MainProcess] Benchmark run finished in {self._total_run_duration:.3f} seconds."
        )
        sys.stdout.flush()
        ...
        if actual_runs_completed != total_expected_runs and not self._errors:
            ...
            print(
                "[MainProcess] Warning: No results collected and no errors reported, but runs were expected.",
                file=sys.stderr,
            )
            sys.stderr.flush()
    ...
    def get_execution_results(self) -> Dict[str, Any]:
        ...
        min_time_str = (
            format(min_time, ".6f").rstrip("0").rstrip(".")
            if actual_runs > 0
            else "N/A"
        )
        max_time_str = (
            format(max_time, ".6f").rstrip("0").rstrip(".")
            if actual_runs > 0
            else "N/A"
        )
        avg_time_str = (
            format(avg_time, ".6f").rstrip("0").rstrip(".")
            if actual_runs > 0
            else "N/A"
        )
    ...
    def get_execution_timeseries(self) -> List[Dict[str, str]]:
        ...
        if not self._run_timestamps:
            if self._errors:
                raise ValueError(
                    "Benchmark completed with errors and produced no successful results."
                )
            else:
                raise ValueError(
                    "Benchmark completed but produced no successful results."
                )
